      ...
      <Combobox.Options className="w-full mt-1 border-gray-300 border-[2px] rounded-md bg-white absolute">
        {filteredUsers.map((user) => (
          <Combobox.Option className="w-full cursor-pointer transition-all duration-75 hover:bg-gray-200 text-base px-2 py-2 border-gray-400 flex gap-x-4 items-center" key={user.value} value={user}>
                <div className=" rounded-full overflow-hidden"><Image src="/picture.jpg" height={40} width={40} className="w-6 h-6"/></div>{user.label}
          </Combobox.Option>
        ))}
      ...
      {value.length > 0 && (
          value.map((user, index) => (
            <div onClick={() => removeElement(user)} className={`${index == 0 ? "border-y-[1px]" : "border-b-[1px] "} w-full text-base cursor-pointer transition-all duration-75 hover:bg-gray-100 border-gray-300 py-2 px-2 items-center flex gap-x-4`}>
              <div key={user.value} className="rounded-full overflow-hidden"><Image src="/picture.jpg" height={40} width={40} className="w-8 h-8"/></div>
              {user.label}
            </div>
          ))
      )
